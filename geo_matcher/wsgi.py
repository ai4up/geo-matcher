...
try:
    ...
    print("Environment variables DATA_PATH, RESULTS_PATH, ANNOTATION_REDUNDANCY and CONSENSUS_MARGIN must be set.", file=sys.stderr)
    sys.exit(1)
except ValueError:
    print("Environment variables ANNOTATION_REDUNDANCY and CONSENSUS_MARGIN must be an integer.", file=sys.stderr)
    sys.exit(1)

app = create_app(data_path, results_path, annotation_redundancy, consensus_margin)
