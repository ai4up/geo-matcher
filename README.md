...
## Install
```bash
pip install git+https://github.com/ai4up/sameplace@main
```

## Usage
Step 1: Create a dataset of potential matching pairs from two datasets:
```bash
sameplace create-labeling-dataset dataset1.parquet dataset2.parquet
```
...
```bash
sameplace create-labeling-dataset \
...
```bash
poetry build
pip install dist/sameplace-*.whl
```
